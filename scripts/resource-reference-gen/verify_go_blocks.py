...
def verify(path, sdk_go_root, sdk_provider_go_root, provider_go_module):
    """Compiles the LITERAL, unmodified page content -- no synthetic
    package/func wrapper, ever. A prior version of this function
    (removed) wrapped a bare, incomplete fragment in a real
    package/func shell before compiling it, which meant "OK" only ever
    proved a RECONSTRUCTION built by this tool compiled, not that the
    real page content, copy-pasted exactly as shown, does. The only
    real fix -- confirmed the founder's own explicit instruction, not
    a guess -- is requiring every generated page to already BE a
    complete, standalone program (build_resource_page_complete's own
    real package main/func main shape, the same one every currently-
    live page already has); this function now trusts that and compiles
    the block exactly as extracted. If a future page is ever
    incomplete again, this fails loudly with a real Go parse error
    instead of silently accommodating it."""
