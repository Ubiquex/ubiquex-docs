...
def render_response_field(f, indent, nested, provider_display):
    ...
    if not nested:
        lines.append(f"{pad}  {field_desc(f, provider_display)}")
    if is_object_ish(t):
...
def build_resource_page(wire, service, local, slug, fields, go, py, ts,
    ...
    go_import_bare = f'import "{go_import_path}"'
    if len(go_import_bare) > 55:
        go_import = f'import (\n        "{go_import_path}"\n    )'
    else:
        go_import = go_import_bare
    if empty:
        go_block = f"""    ```go
    {go_import}
...
{go_field_lines}
    ...
    ts_field_lines = "\n".join(
        f"      {camel(f['WireName'])}: {literal_ts(f)}," for f in example_fields
    )
    if ts_published:
        ts_import_path = f'jsr:@ubx/sdk-{schema_name}/{schema_name}/{ts["service_dir"]}/{os.path.splitext(os.path.basename(ts["file"]))[0]}'
        ts_import_line = f'import {{ {ts["binding"]} }} from "{ts_import_path}";'
        if len(ts_import_line) > 68:
            ts_import = f'import {{ {ts["binding"]} }} from\n      "{ts_import_path}";'
        else:
            ts_import = ts_import_line
    else:
        ts_local_path = f'./{ts_repo}/{ts["file"]}'
        ts_import_line = f'import {{ {ts["binding"]} }} from "{ts_local_path}";'
        if len(ts_import_line) > 68:
            ts_import = f'import {{ {ts["binding"]} }} from\n      "{ts_local_path}";'
        else:
            ts_import = ts_import_line
    resource_call_open = "{}" if empty else f"{{\n{ts_field_lines}\n    }}"
    if ts_published:
        ts_block = f"""    ```typescript
    {ts_import}

    resource({ts["binding"]}, "example", {resource_call_open});
    ```"""
    else:
        ts_block = f"""    ```typescript
    // git clone https://github.com/Ubiquex/{ts_repo}.git
    {ts_import}
...
`{wire}` -- real, typed bindings generated directly from the
`hashicorp/{schema_name}` provider schema, in every SDK language.
...
def generate_mechanical_provider(docs_root, scratch_dir, provider, schema_name, provider_display,
    ...
    for service in sorted(by_service.keys()):
        ...
        if len(items) > 1 and not index_collision:
            card_body = f"""---
title: "{title}"
description: "hashicorp/{schema_name} {title} resource types."
---
...
description: "hashicorp/{schema_name} resource types, grouped by real service."
---

The real `hashicorp/{schema_name}` provider covers {len(schema)} resource types
