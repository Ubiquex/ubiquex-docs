...
# the fuller explanation moved to a real, once-per-page <Note> (see
# ai_inferred_note below), never repeated per field.
...
def any_ai_inferred(fields):
    """True if `fields` (a real ir.Field list, top-level OR nested --
    walks Object/List/Set/Map.Element the same way render_response_field
    itself does) contains at least one real AI-inferred field anywhere,
    not just at the top level -- confirmed live for Datadog: ALL of its
    real AI-inferred fields are nested, a top-level-only check would
    have silently suppressed the page note on every one of its pages."""
    for f in fields:
        if f.get("DescriptionSource") == "ai-inferred":
            return True
        if any_ai_inferred(object_fields_of(f["Type"])):
            return True
    return False


# The fuller explanation, real and honest, shown at most ONCE per page
# -- immediately before "## Input properties" -- rather than once per
# field. Mintlify's own real, already-proven-in-this-corpus <Note>
# component (render_response_field's own cycle-detection/depth-backstop
# messages already use it, real precedent, not a guess).
AI_INFERRED_NOTE = """<Note>
...
def ai_inferred_note(input_fields, output_fields):
    if any_ai_inferred(input_fields) or any_ai_inferred(output_fields):
        return AI_INFERRED_NOTE
    return ""
