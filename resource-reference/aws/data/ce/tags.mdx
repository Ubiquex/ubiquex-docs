...
<Tabs>
  ...
  <Tab title="Python">
    ...
    from ubx.aws.data.ce.tags import Tags_TimePeriod, Tags_Filter, Tags_Filter_And, Tags_SortBy

    def describe():
        ubx.intent("look up aws_ce_tags")

        ubx.data(Tags, "example", TagsConfig(
            time_period=Tags_TimePeriod(end="example", start="example"),
            billing_view_arn="arn:aws:iam::123456789012:policy/example",
            filter=Tags_Filter(and_=[Tags_Filter_And()]),
            max_results=1,
            search_string="example",
            sort_by=[Tags_SortBy(key="example")],
            tag_key="example",
        ))
