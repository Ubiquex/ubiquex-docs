---
title: "aws_ce_dimension_values"
description: "`aws_ce_dimension_values` reads the real, distinct values available for a Cost Explorer cost and usage dimension."
---
...
<Tabs>
  <Tab title="Go">
    ...
    func main() {
    	ubx.Main(ubx.Stack("example", func() {
    		ubx.Intent(ubx.IntentInfo{Summary: "look up aws_ce_dimension_values"})

    		ubx.Data(ce.DimensionValues, "example", ce.DimensionValuesConfig{
    			Dimension:      "example",
  ...
  <Tab title="TypeScript">
    ...
    import { DimensionValues } from "@ubx/sdk-aws/aws/data/ce/dimension_values";

    export default stack("example", () => {
      intent({ summary: "look up aws_ce_dimension_values" });

      data(DimensionValues, "example", {
        dimension: "example",
  ...
  <Tab title="Python">
    ...
    from ubx.aws.data.ce import DimensionValues, DimensionValuesConfig
    from ubx.aws.data.ce.dimension_values import DimensionValues_TimePeriod, DimensionValues_Filter, DimensionValues_Filter_And, DimensionValues_SortBy

    def describe():
        ubx.intent("look up aws_ce_dimension_values")

        ubx.data(DimensionValues, "example", DimensionValuesConfig(
            dimension="example",
            time_period=DimensionValues_TimePeriod(end="example", start="example"),
            billing_view_arn="arn:aws:iam::123456789012:policy/example",
            context="example",
            filter=DimensionValues_Filter(and_=[DimensionValues_Filter_And()]),
            max_results=1,
            search_string="example",
            sort_by=[DimensionValues_SortBy(key="example")],
        ))
...
<ResponseField name="dimension" type="string" required>
  The real dimension this lookup is scoped to. **(AI-inferred)** Required lookup argument.
</ResponseField>
...
<ResponseField name="dimension_values" type="list(object)">
  The real values of the dimension included in this result. **(AI-inferred)** Returned by the lookup.
</ResponseField>
