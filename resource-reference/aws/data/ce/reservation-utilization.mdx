...
<Tabs>
  ...
  <Tab title="Python">
    ...
    from ubx.aws.data.ce.reservation_utilization import ReservationUtilization_TimePeriod, ReservationUtilization_Filter, ReservationUtilization_Filter_And, ReservationUtilization_GroupBy, ReservationUtilization_SortBy

    def describe():
        ubx.intent("look up aws_ce_reservation_utilization")

        ubx.data(ReservationUtilization, "example", ReservationUtilizationConfig(
            time_period=ReservationUtilization_TimePeriod(end="example", start="example"),
            filter=ReservationUtilization_Filter(and_=[ReservationUtilization_Filter_And()]),
            granularity="example",
            group_by=[ReservationUtilization_GroupBy(key="example")],
            max_results=1,
            sort_by=ReservationUtilization_SortBy(key="example"),
        ))
