---
title: "aws_rds_dbinstance"
description: "`aws_rds_dbinstance` creates a real managed relational database instance, MySQL, PostgreSQL, MariaDB, Oracle, SQL Server..."
---

`aws_rds_dbinstance` creates a real managed relational database instance, MySQL, PostgreSQL, MariaDB, Oracle, SQL Server, or an Aurora cluster's own compute, the core resource RDS provisions and patches on the customer's behalf.
...
<Tabs>
  <Tab title="Go">
    ...
    func main() {
    	ubx.Main(ubx.Stack("payments", func() {
    		ubx.Intent(ubx.IntentInfo{Summary: "payments own dbinstance"})

    		ubx.Resource(rds.Dbinstance, "ci-runner", rds.DbinstanceConfig{
    			AdditionalStorageVolumes:           []map[string]any{{"allocated_storage": "example"}},
    			AllocatedStorage:                   "example",
    			AllowMajorVersionUpgrade:           true,
    			ApplyImmediately:                   true,
    			AssociatedRoles:                    []map[string]any{{"feature_name": "example-feature-name"}},
    			AutoMinorVersionUpgrade:            true,
    			AutomaticBackupReplicationKmsKeyId: "example",
    			AutomaticBackupReplicationRegion:   "example",
    		})
    	}))
    }
    ```
  ...
  <Tab title="TypeScript">
    ...
    import { Dbinstance } from "./local-sdk/aws/sdk/typescript/aws/rds/dbinstance.ts";

    export default stack("payments", () => {
      intent({ summary: "payments own dbinstance" });

      resource(Dbinstance, "ci-runner", {
        additionalStorageVolumes: [{ allocatedStorage: "example" }],
        allocatedStorage: "example",
        allowMajorVersionUpgrade: true,
        applyImmediately: true,
  ...
  <Tab title="Python">
    ...
    from ubx.aws.rds import Dbinstance, DbinstanceConfig

    def describe():
        ubx.intent("payments own dbinstance")

        ubx.resource(Dbinstance, "ci-runner", DbinstanceConfig(
            additional_storage_volumes=[{"allocated_storage": "example"}],
            allocated_storage="example",
            allow_major_version_upgrade=True,
            apply_immediately=True,
            associated_roles=[{"feature_name": "example-feature-name"}],
            auto_minor_version_upgrade=True,
            automatic_backup_replication_kms_key_id="example",
            automatic_backup_replication_region="example",
        ))
  ...
  <Tab title="Markdown">
    ```
    Create aws_rds_dbinstance, with additional storage volumes a set
    value, allocated storage example, allow major version upgrade
    true, apply immediately true, associated roles a set value, auto
    minor version upgrade true, automatic backup replication kms key
    id example, automatic backup replication region example.
    ```
  </Tab>
</Tabs>
...
<ResponseField name="additional_storage_volumes" type="list(object)">
  The additional storage volumes associated with the DB instance. RDS supports additional storage volumes for RDS for Oracle and RDS for SQL Server. Optional.
  <Expandable title="properties">
    <ResponseField name="allocated_storage" type="string">
      The amount of storage, in gibibytes (GiB), to allocate for this additional storage volume attached to the RDS DB instance. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="iops" type="number">
      The number of provisioned IOPS to allocate for the additional storage volume on the RDS instance, applicable when the volume's storage type is a provisioned-IOPS type such as io1 or io2. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="max_allocated_storage" type="number">
      The maximum storage size, in gigabytes, to which this additional storage volume can be automatically scaled when automatic storage expansion is enabled for the DB instance. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="storage_throughput" type="number">
      Specifies the storage throughput in MiB/s for the additional storage volume when using the gp3 storage type. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="storage_type" type="string">
      Optional; if omitted, computed by AWS.
    </ResponseField>
    <ResponseField name="volume_name" type="string">
      Specifies the name of an additional storage volume for the DB instance, which must be unique among all additional storage volumes on the instance. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="allocated_storage" type="string">
  The amount of storage in gibibytes (GiB) to be initially allocated for the database instance. If any value is set in the ``Iops`` parameter, ``AllocatedStorage`` must be at least 100 GiB, which corresponds to the minimum Iops value of 1,000. If you increase the ``Iops`` value (in 1,000 IOPS increments), then you must also increase the ``AllocatedStorage`` value (in 100-GiB increments). *Amazon Aurora* Not applicable. Aurora cluster volumes automatically grow as the amount of data in your database increases, though you are only charged for the space that you use in an Aurora cluster volume. *Db2* Constraints to the amount of storage for each storage type are the following: + General Purpose (SSD) storage (gp3): Must be an integer from 20 to 64000. + Provisioned IOPS storage (io1): Must be an integer from 100 to 64000. *MySQL* Constraints to the amount of storage for each storage type are the following: + General Purpose (SSD) storage (gp2): Must be an integer from 20 to 65536. + Provisioned IOPS storage (io1): Must be an integer from 100 to 65536. + Magnetic storage (standard): Must be an integer from 5 to 3072. *MariaDB* Constraints to the amount of storage for each storage type are the following: + General Purpose (SSD) storage (gp2): Must be an integer from 20 to 65536. + Provisioned IOPS storage (io1): Must be an integer from 100 to 65536. + Magnetic storage (standard): Must be an integer from 5 to 3072. *PostgreSQL* Constraints to the amount of storage for each storage type are the following: + General Purpose (SSD) storage (gp2): Must be an integer from 20 to 65536. + Provisioned IOPS storage (io1): Must be an integer from 100 to 65536. + Magnetic storage (standard): Must be an integer from 5 to 3072. *Oracle* Constraints to the amount of storage for each storage type are the following: + General Purpose (SSD) storage (gp2): Must be an integer from 20 to 65536. + Provisioned IOPS storage (io1): Must be an integer from 100 to 65536. + Magnetic storage (standard): Must be an integer from 10 to 3072. *SQL Server* Constraints to the amount of storage for each storage type are the following: + General Purpose (SSD) storage (gp2): + Enterprise and Standard editions: Must be an integer from 20 to 16384. + Web and Express editions: Must be an integer from 20 to 16384. + Provisioned IOPS storage (io1): + Enterprise and Standard editions: Must be an integer from 20 to 16384. + Web and Express editions: Must be an integer from 20 to 16384. + Magnetic storage (standard): + Enterprise and Standard editions: Must be an integer from 20 to 1024. + Web and Express editions: Must be an integer from 20 to 1024. Optional.
</ResponseField>

<ResponseField name="allow_major_version_upgrade" type="bool">
  A value that indicates whether major version upgrades are allowed. Changing this parameter doesn't result in an outage and the change is asynchronously applied as soon as possible. Constraints: Major version upgrades must be allowed when specifying a value for the ``EngineVersion`` parameter that is a different major version than the DB instance's current version. Optional.
</ResponseField>

<ResponseField name="apply_immediately" type="bool">
  Specifies whether changes to the DB instance and any pending modifications are applied immediately, regardless of the ``PreferredMaintenanceWindow`` setting. If set to ``false``, changes are applied during the next maintenance window. Until RDS applies the changes, the DB instance remains in a drift state. As a result, the configuration doesn't fully reflect the requested modifications and temporarily diverges from the intended state. In addition to the settings described in [Modifying a DB instance](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Overview.DBInstance.Modifying.html), this property also determines whether the DB instance reboots when a static parameter is modified in the associated DB parameter group. Default: ``true`` Optional.
</ResponseField>

<ResponseField name="associated_roles" type="list(object)">
  The IAMlong (IAM) roles associated with the DB instance. *Amazon Aurora* Not applicable. The associated roles are managed by the DB cluster. Optional.
  <Expandable title="properties">
    <ResponseField name="feature_name" type="string">
      The name of the feature (e.g., S3_INTEGRATION) that the associated IAM role enables for the DB instance, such as allowing Amazon S3 integration. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="role_arn" type="string">
      The ARN of the IAM role to associate with the DB instance, enabling the instance to make API calls to other AWS services like S3 or Lambda on your behalf. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="auto_minor_version_upgrade" type="bool">
  A value that indicates whether minor engine upgrades are applied automatically to the DB instance during the maintenance window. By default, minor engine upgrades are applied automatically. Optional.
</ResponseField>

<ResponseField name="automatic_backup_replication_kms_key_id" type="string">
  The AWS KMS key identifier for encryption of the replicated automated backups. The KMS key ID is the Amazon Resource Name (ARN) for the KMS encryption key in the destination AWS-Region, for example, ``arn:aws:kms:us-east-1:123456789012:key/AKIAIOSFODNN7EXAMPLE``. Optional.
</ResponseField>

<ResponseField name="automatic_backup_replication_region" type="string">
  The AWS-Region associated with the automated backup. Optional.
</ResponseField>

<ResponseField name="automatic_backup_replication_retention_period" type="number">
...
<ResponseField name="dedicated_log_volume" type="bool">
  Indicates whether the DB instance has a dedicated log volume (DLV) enabled. Optional.
</ResponseField>

<ResponseField name="delete_automated_backups" type="bool">
  A value that indicates whether to remove automated backups immediately after the DB instance is deleted. This parameter isn't case-sensitive. The default is to remove automated backups immediately after the DB instance is deleted. *Amazon Aurora* Not applicable. When you delete a DB cluster, all automated backups for that DB cluster are deleted and can't be recovered. Manual DB cluster snapshots of the DB cluster are not deleted. Optional.
</ResponseField>

<ResponseField name="deletion_protection" type="bool">
  Specifies whether the DB instance has deletion protection enabled. The database can't be deleted when deletion protection is enabled. By default, deletion protection isn't enabled. For more information, see [Deleting a DB Instance](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_DeleteInstance.html). This setting doesn't apply to Amazon Aurora DB instances. You can enable or disable deletion protection for the DB cluster. For more information, see ``CreateDBCluster``. DB instances in a DB cluster can be deleted even when deletion protection is enabled for the DB cluster. Optional.
</ResponseField>
...
<ResponseField name="master_user_secret" type="object">
  ...
  <Expandable title="properties">
    <ResponseField name="kms_key_id" type="string">
      The AWS KMS key identifier that is used to encrypt the secret. Optional.
    </ResponseField>
    <ResponseField name="secret_arn" type="string">
      The Amazon Resource Name (ARN) of the secret. This parameter is a return value that you can retrieve using the ``Fn::GetAtt`` intrinsic function. For more information, see [Return values](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-rds-dbinstance.html#aws-resource-rds-dbinstance-return-values). Optional.
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="percent_progress" type="string">
  The progress, as a percentage, of a major version upgrade or database restoration for the DB instance, as reported by the AWS RDS API. Computed by AWS after `ship`. **(AI-inferred)**
</ResponseField>

<ResponseField name="read_replica_dbcluster_identifiers" type="list(string)">
  Lists the identifiers of the Aurora DB clusters for which this DB instance is a read replica, as reported by RDS after creation. Computed by AWS after `ship`. **(AI-inferred)**
</ResponseField>

<ResponseField name="read_replica_dbinstance_identifiers" type="list(string)">
  A list of identifiers for any read replicas attached to this RDS DB instance, as reported by the AWS RDS describe-db-instances API. Computed by AWS after `ship`. **(AI-inferred)**
</ResponseField>

<ResponseField name="resume_full_automation_mode_time" type="string">
  The timestamp indicating when the RDS Custom DB instance will automatically resume full automation mode after a pause, present only when the instance's automation is in a paused state. Computed by AWS after `ship`. **(AI-inferred)**
</ResponseField>

<ResponseField name="secondary_availability_zone" type="string">
  The Availability Zone of the standby DB instance in a Multi-AZ deployment, automatically populated by AWS and read-only. Computed by AWS after `ship`. **(AI-inferred)**
</ResponseField>

<ResponseField name="status_infos" type="list(object)">
  The status_infos field is a read-only list of status information objects for the DB instance, each containing a status type, status value, a descriptive message, and a boolean indicating whether the status is normal. Computed by AWS after `ship`. **(AI-inferred)**
  <Expandable title="properties">
    <ResponseField name="message" type="string">
      The status message returned by RDS for a DB instance's status information entry, such as the reason behind a read replica or maintenance status. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="normal" type="bool">
      Indicates whether the RDS DB instance is operating normally with no issues for this particular status info entry, as returned by the RDS DescribeDBInstances API's DBInstanceStatusInfo object. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="status" type="string">
      The status of a read replica's replication, as reported in the RDS API's StatusInfos list. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
    <ResponseField name="status_type" type="string">
      Indicates the type of an RDS DB instance status entry (e.g., 'read replication') within the status_infos list, which categorizes a particular status message. Optional; if omitted, computed by AWS. **(AI-inferred)**
    </ResponseField>
  </Expandable>
</ResponseField>
